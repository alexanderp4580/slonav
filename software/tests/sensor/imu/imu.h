...
// TODO: replace includes with mbed i2c library
...
{
    protected:
    // The imu class uses this pointer to print debug messages via the serial 
    // port and I2C communcations with the sensor.
    ...
    // line where this constructor is called, the compiler will just fill in 
    // "NULL". In this case that has the effect of turning off diagnostic 
    // printouts.

    typedef enum imu_reg
    {
    ...
    // Contructor declaration with emstream* = NULL being the default input if 
    // nothing entered.
    imu(emstream* = NULL);
